...
void __fastcall__ pal_bg_high(const char *data);

//set spr palette only, data is 16 bytes array

void __fastcall__ pal_spr(const char *data);
void __fastcall__ pal_spr_high(const char *data);

// Update one row of the palettes (4-bit mode only)
void __fastcall__ pal_upd_single(unsigned char row);

// Palette update routines
void pal_wait_update();
unsigned char pal_get_update();
